...
Take screenshots or videos (MOV format) of macOS windows from the command line.
...
#+begin_src shell
  ...
  OPTIONS:
    ...
    -r, --record <app name or window id>
                            Start recording (MOV format, streams directly to disk).
...
Use =--record= + window number to start recording a video (MOV format). The video file is created immediately and frames are streamed directly to disk.
...
#+begin_src sh
  ...
  Recording to: ~/Desktop/2023-04-14-08:21:45-Emacs.mov
  ^C
  Saving...
...
#+begin_src sh
  ~ $ macosrec --record emacs --output ~/Videos/my-recording.mov
  Recording to: ~/Videos/my-recording.mov
  ^C
  Saving...
  ~/Videos/my-recording.mov
#+end_src
